...
COLOR_BASE = ROOT / "color.json"
COLOR_LIGHT = ROOT / "color_light.json"
COLOR_DARK = ROOT / "color_dark.json"
DATA_VIS_LIGHT = ROOT / "data_vis_light.json"
DATA_VIS_DARK = ROOT / "data_vis_dark.json"
OPACITY_BASE = ROOT / "opacity.json"
OPACITY_SYS = ROOT / "opacity2.json"
THEME_FILE = ROOT / "ThemeV1.kt"
OUT_FILE = ROOT / "ThemeV1.generated.kt"
...
def build_base_opacity(opacity_sys_json: dict) -> str:
    """
    From opacity.2json.json -> object BaseOpacity { val opacity40: Float = Opacity.opacity40 ... }
    """
...
def main():
    base_json = json.loads(COLOR_BASE.read_text(encoding="utf-8"))
    light_json = json.loads(COLOR_LIGHT.read_text(encoding="utf-8"))
    dark_json = json.loads(COLOR_DARK.read_text(encoding="utf-8"))
    data_vis_light_json = json.loads(DATA_VIS_LIGHT.read_text(encoding="utf-8"))
    data_vis_dark_json = json.loads(DATA_VIS_DARK.read_text(encoding="utf-8"))
    opacity_json = json.loads(OPACITY_BASE.read_text(encoding="utf-8"))
    opacity_sys_json = json.loads(OPACITY_SYS.read_text(encoding="utf-8"))
    ...
    theme_src = THEME_FILE.read_text(encoding="utf-8")
    ...
    OUT_FILE.write_text(header_block + suffix3, encoding="utf-8")
