...
OPACITY_SYS = "../data/01_system/opacity2.json"  # not used for values, kept if you need it later
...
def opacity_ref_to_kotlin(ref: str) -> Optional[str]:
    # {opacity.20} -> BaseOpacity.opacity20
    ref = ref.strip()
...
# ---------- opacity: single BaseOpacity ----------

def build_base_opacity_from_base(opacity_json: dict) -> str:
    """
    From opacity.json -> object BaseOpacity { val opacity0: Float = 0.0f ... }
    """
...
def collect_field_specs(
    ...
    for path in ordered_paths:
        base_name = path_to_field_name(path)
        if not base_name:
            continue
...
def inject_sets_into_palettes(src: str, field_specs):
    ...
    def build_set_block(prop_name: str, class_name: str, specs, use_light: bool):
        lines = [f"    {prop_name} = {class_name}("]
    ...
    def inject_into_palette(palette_name: str, text: str, use_light: bool):
        ...
        if set1:
            extra_blocks.append(
                build_set_block("colorSetOne", "ColorSetOne", set1, use_light)
            )
        if set2:
            extra_blocks.append(
                build_set_block("colorSetTwo", "ColorSetTwo", set2, use_light)
            )
        if set3:
            extra_blocks.append(
                build_set_block("colorSetThree", "ColorSetThree", set3, use_light)
            )
...
# ---------- strip any existing object Opacity ----------

OPACITY_OBJECT_RE = re.compile(r"object\s+Opacity\s*\{.*?\}", re.DOTALL)

def remove_legacy_opacity_object(kotlin_src: str) -> str:
    return OPACITY_OBJECT_RE.sub("", kotlin_src)
...
def main():
    ...
    base_opacity_obj = build_base_opacity_from_base(opacity_json)
    ...
    # Remove any legacy object Opacity from original file
    theme_src = remove_legacy_opacity_object(theme_src)
